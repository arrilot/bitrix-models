...
## Работа со связными данными (WIP)

Помимо работы с отдельными Битрикс-моделями, также имеется возможность объединять связные данные, что делает их легко-доступными для получения через основные объекты данных. Например, товар связан с вопросами о товарах. С помощью объявления этой связи вы можете получить возможность доступа к информации вопросах с помощью выражения $product->questions, которое возвращает информацию о вопросах в виде колелкции объектов класса Question (дочерний класс BaseBitrixModel).

### Объявление связей

Для работы со связными данными необходимо объявить связи в классе модели. Это делаетмя объявлением методов получения связных данных для каждой интересующей вас связи как показано ниже:
...
- кратность связи: указывается с помощью вызова метода hasMany() или метода hasOne(). В вышеприведённом примере вы можете легко увидеть в объявлениях связей, что това может иметь много вопросов, но один бренд.
- название связного класса: указывается в качестве первого параметра для метода hasMany() или для метода hasOne().
- связь между двумя типами данных: указываются столбцы с помощью которых два типа данных связаны. Значения массива - это столбцы основного объекта данных, в то время как ключи массива - столбцы связанных данных.
...
// При повторном обращении запросы уже не выпонляются 
$product->brand;
$product->questions;
```
...
В примере из раздела "Доступ к связным данным" используется отложенная загрузка. Т.е. данные загружаются при первом обращении к ним. В случае работы с коллекцией продуктов можно получить множество запросов к БД
...
- В качестве параметра принимает строку - название связи или массив таких строк `->with(['brand', 'questions'])`.
...
```php
->with([
    'questions' => function (BaseQuery $query){
        $query->filter(['ACTIVE' => 'Y'])
    },
    'question.answers'
])
