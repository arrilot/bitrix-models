...
class Product extends ElementModel
{
     ...
     * @return int
     */
    protected static $iblockId = PRODUCT_IBLOCK_ID;
}
```

Для работы модели необходимо лишь задать.
реализовать `iblockId()` возвращающий ID информационного блока.
Для юзеров не требуется и этого.

> Если вы не хотите привязываться к ID инфоблоков, можно переопределить метод `public static iblockId()` и получать в нём ID инфоблока например по коду.
